-- with this you can turn on/off specific hovnosense components, note: you can also turn these off while the script is running by using events, see examples for such below
Components = {
	Teleport = true,
	GodMode = true,
	Speedhack = false,
...
These can be used by triggering them like following:
	TriggerEvent("anticheat:SetComponentStatus", "Teleport", false)
...
Citizen.CreateThread(function()
	...
	function SendWebhookMessage(wh,message)
		webhook = GetConvar("hs_webhook", "none")
		if wh ~= "none" then
	...
	RegisterServerEvent('anticheat:SpeedFlag')
	AddEventHandler('anticheat:SpeedFlag', function(rounds, roundm)
		if Components.Speedhack and not IsPlayerAceAllowed(source,"anticheat.bypass") then
	...
	RegisterServerEvent('anticheat:NoclipFlag')
	AddEventHandler('anticheat:NoclipFlag', function(distance)
		if Components.Speedhack and not IsPlayerAceAllowed(source,"anticheat.bypass") then
			local license, steam = GetPlayerNeededIdentifiers(source)
			local name = GetPlayerName(source)
	...
	AddEventHandler('anticheat:CustomFlag', function(reason,extrainfo)
		if Components.CustomFlag and not IsPlayerAceAllowed(source,"anticheat.bypass") then
			local license, steam = GetPlayerNeededIdentifiers(source)
			local name = GetPlayerName(source)
	...
	RegisterServerEvent('anticheat:HealthFlag')
	AddEventHandler('anticheat:HealthFlag', function(invincible,oldHealth, newHealth, curWait)
		if Components.GodMode and not IsPlayerAceAllowed(source,"anticheat.bypass") then
			local license, steam = GetPlayerNeededIdentifiers(source)
			local name = GetPlayerName(source)
	...
	AddEventHandler('anticheat:JumpFlag', function(jumplength)
		if Components.SuperJump and not IsPlayerAceAllowed(source,"anticheat.bypass") then
			local license, steam = GetPlayerNeededIdentifiers(source)
			local name = GetPlayerName(source)
	...
	RegisterServerEvent('anticheat:WeaponFlag')
	AddEventHandler('anticheat:WeaponFlag', function(weapon)
		if Components.WeaponBlacklist and not IsPlayerAceAllowed(source,"anticheat.bypass") then
			local license, steam = GetPlayerNeededIdentifiers(source)
			local name = GetPlayerName(source)
			...
			TriggerClientEvent("anticheat:RemoveInventoryWeapons", source) 
		end
	end)

	RegisterServerEvent('anticheat:CarFlag')
	AddEventHandler('anticheat:CarFlag', function(car)
		if Components.CarBlacklist and not IsPlayerAceAllowed(source,"anticheat.bypass") then
			local license, steam = GetPlayerNeededIdentifiers(source)
			local name = GetPlayerName(source)
...
Citizen.CreateThread( function()
	local updatePath = "/Bluethefurry/hovnosense-anticheat"
	local resourceName = "anticheat ("..GetCurrentResourceName()..")"
